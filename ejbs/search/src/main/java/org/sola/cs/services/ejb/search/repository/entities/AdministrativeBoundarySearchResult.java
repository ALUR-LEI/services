...
public class AdministrativeBoundarySearchResult extends AbstractReadOnlyEntity {
    ...
    private static final String SELECT_PART = ""
            ...
            + "  SELECT id, name, type_code, authority_name, authority_code, parent_id, status_code, 1 as level, array[ROW_NUMBER() OVER (ORDER BY name)] AS path\n "
            + "  FROM opentenure.administrative_boundary \n "
            + "  WHERE parent_id is null\n "
            + " UNION \n "
            + "	 SELECT b.id, b.name, b.type_code, b.authority_name, b.authority_code, b.parent_id, b.status_code, ab.level + 1 as level, ab.path || (ROW_NUMBER() OVER (ORDER BY b.name)) AS path\n "
            + "  FROM opentenure.administrative_boundary b inner join all_administrative_boundaries ab on b.parent_id = ab.id\n "
            + ")\n "
            + "SELECT b.id, b.name, b.type_code, get_translation(bt.display_value, #{" + CommonSqlProvider.PARAM_LANGUAGE_CODE + "}) as type_name, b.authority_name, b.authority_code, b.parent_id, b.status_code, get_translation(bs.display_value, #{" + CommonSqlProvider.PARAM_LANGUAGE_CODE + "}) as status_name, b.level, b.path::varchar \n"
    ...
    public static final String QUERY_GET_PARENTS = ""
            ...
            + ") \n"
            + "SELECT b.path::varchar, b.id, b.name, b.type_code, get_translation(bt.display_value, #{" + CommonSqlProvider.PARAM_LANGUAGE_CODE + "}) as type_name, b.authority_name, b.authority_code, b.parent_id, b.status_code, get_translation(bs.display_value, #{" + CommonSqlProvider.PARAM_LANGUAGE_CODE + "}) as status_name, b.level \n"
            + "FROM (all_administrative_boundaries b INNER JOIN opentenure.administrative_boundary_type bt ON b.type_code = bt.code) INNER JOIN opentenure.administrative_boundary_status bs ON b.status_code = bs.code \n"
