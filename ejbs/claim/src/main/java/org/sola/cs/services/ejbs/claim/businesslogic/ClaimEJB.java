...
public class ClaimEJB extends AbstractEJB implements ClaimEJBLocal {
    ...
    @Override
    @RolesAllowed({RolesConstants.CS_RECORD_CLAIM, RolesConstants.CS_REVIEW_CLAIM, RolesConstants.CS_MODERATE_CLAIM, RolesConstants.CS_ISSUE_CERTIFICATE})
...
//            throw new SOLAException(ServiceMessage.BOUNDARY_ID_IS_NULL);
    ...
    public boolean issueCertificate(String claimId, final String languageCode) {
        if (StringUtility.isEmpty(claimId)) {
            return false;
        }

        Claim claim = getRepository().getEntity(Claim.class, claimId);
        canIssueCertificate(claim, true);

        // Check documents
        List<SourceType> docTypes = getDocumentTypesForIssuance(languageCode);
...
// change 8 March  as error from claim.getAttachments following getDocumentTypesForIssuance
    ...
    @Override
    @RolesAllowed({RolesConstants.CS_MODERATE_CLAIM, RolesConstants.CS_PRINT_CERTIFICATE, RolesConstants.CS_ISSUE_CERTIFICATE})
    public boolean renewCertificate(String claimId, final String languageCode) {
        if (StringUtility.isEmpty(claimId)) {
            return false;
        }

        Claim claim = getRepository().getEntity(Claim.class, claimId);
        canRenewCertificate(claim, true);
        ...
        if (docTypes.size() > 0) {
            ...
            if (missingDocs.length() > 0) {
                // Throw error
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_CANT_RENEW_FOUND_MISSING_DOCS, new Object[]{missingDocs});
            }
        }
   ...
   @Override
    ...
    public boolean cancelCertificate(String claimId, final String languageCode) {
        ...
        Claim claim = getRepository().getEntity(Claim.class, claimId);
        canCancelCertificate(claim, true);
...
// change 8 March  as error from claim.getAttachments following getDocumentTypesForIssuance
            ...
            if (missingDocs.length() > 0) {
                // Throw error
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_CANT_CANCEL_FOUND_MISSING_DOCS, new Object[]{missingDocs});
            }
        }
    ...
    private boolean canEditClaim(Claim claim, boolean throwException) {
        ...
        if (claim.getChallengeExpiryDate() != null
            ...
            // If claim has CREATED status, check its recorder
    ...
    private boolean changeClaimStatus(String claimId, Claim challengedClaim, String statusCode, String rejectionCode) {
        ...
        if (statusCode.equalsIgnoreCase(ClaimStatusConstants.RENEWED)) {
            claimStatusChanger.setIssuanceDate(Calendar.getInstance().getTime());
        }
    ...
    public boolean canIssueCertificate(String id) {
        return canIssueCertificate(getRepository().getEntity(Claim.class, id), false);
    }

    private boolean canIssueCertificate(Claim claim, boolean throwException) {
        if (claim == null) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_NOT_FOUND);
            }
        ...
        // Check user role
        if (!isInRole(RolesConstants.CS_ISSUE_CERTIFICATE)) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.EXCEPTION_INSUFFICIENT_RIGHTS);
            }
            return false;
    ...
    public boolean canRenewCertificate(String id) {
        return canRenewCertificate(getRepository().getEntity(Claim.class, id), false);
    }

    private boolean canRenewCertificate(Claim claim, boolean throwException) {
        if (claim == null) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_NOT_FOUND);
            }
        ...
        if (!claim.getStatusCode().equalsIgnoreCase(ClaimStatusConstants.ISSUED)) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_CANT_RENEW);
            }
            return false;
        }
        return true;
    ...
    public boolean canCancelCertificate(String claimId) {
        return canCancelCertificate(getRepository().getEntity(Claim.class, claimId), false);
    }

    private boolean canCancelCertificate(Claim claim, boolean throwException) {
        if (claim == null) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_NOT_FOUND);
            }
        ...
        // Check claim is assigned to the modifying user
        String userName = getUserName();
        if (!StringUtility.empty(claim.getAssigneeName()).equalsIgnoreCase(userName)) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.OT_WS_CLAIM_ASSIGNED_TO_OTHER_USER);
        ...
        // Check user role
        if (!isInRole(RolesConstants.CS_CANCEL_CERTIFICATE)) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.EXCEPTION_INSUFFICIENT_RIGHTS);
            }
            return false;
    ...
    private boolean canAssignClaim(Claim claim, boolean throwException) {
        ...
        // Check user role
        if (!isInRole(RolesConstants.CS_MODERATE_CLAIM, RolesConstants.CS_REVIEW_CLAIM, RolesConstants.CS_ISSUE_CERTIFICATE, RolesConstants.CS_PRINT_CERTIFICATE, RolesConstants.CS_CANCEL_CERTIFICATE)) {
            if (throwException) {
                throw new SOLAException(ServiceMessage.EXCEPTION_INSUFFICIENT_RIGHTS);
            }
            return false;
...
//        }
 ...
 //       }
    ...
    @Override
    @RolesAllowed({RolesConstants.CS_MODERATE_CLAIM})
    public boolean reviseAdministrativeBoundary(String boundaryId) {
        if (StringUtility.isEmpty(boundaryId)) {
            throw new SOLAException(ServiceMessage.GENERAL_OBJECT_IS_NULL);
        }
        ...
        // Check status
        if (dbBoundary.getStatusCode().equalsIgnoreCase(AdministrativeBoundaryStatus.STATUS_PENDING)) {
            throw new SOLAException(ServiceMessage.OT_WS_BOUNDARY_CANT_REVISE, new Object[]{StringUtility.empty(dbBoundary.getName())});
        }

        dbBoundary.setStatusCode(AdministrativeBoundaryStatus.STATUS_PENDING);
