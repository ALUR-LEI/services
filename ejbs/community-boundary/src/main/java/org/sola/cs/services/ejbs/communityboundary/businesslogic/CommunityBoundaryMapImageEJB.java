...
import org.sola.cs.services.ejbs.communityboundary.entities.AdministrativeBoundarySpatial;
...
public class CommunityBoundaryMapImageEJB extends AbstractEJB implements CommunityBoundaryMapImageEJBLocal {
    ...
    /**
     * Returns spatial boundaries by boundary id
     *
     * @param boundaryId Boundary ID
     * @param srid SRID
     * @return
     */
    private List<AdministrativeBoundarySpatial> getSpatialBoundariesByBoundary(String boundaryId, int srid) {
        HashMap params = new HashMap();
        params.put(CommonSqlProvider.PARAM_QUERY, AdministrativeBoundarySpatial.QUERY_GET_BY_ID);
        params.put(AdministrativeBoundarySpatial.PARAM_BOUNDARY_ID, boundaryId);
        params.put(AdministrativeBoundarySpatial.PARAM_CUSTOM_SRID, srid);
    ...
    private MapContent getBoundaryMap(String boundaryId, int width, int height) {
        try {
            ...
            URL sldURL = CommunityBoundaryMapImageEJB.class.getResource(resourcesPath + "cert_parcel.xml");
            ...
            throw new SOLAException(ServiceMessage.OT_WS_BOUNDARY_FAILED_TO_CREATE_MAP);
        }
    }
    ...
    public BufferedImage getBoundaryMapImage(String boundaryId, int width, int height, boolean drawScale, String scaleLabel) {
        MapContent map = getBoundaryMap(boundaryId, width - mapMargin, height - mapMargin);

        if (map == null) {
            return null;
        }
        return getBoundaryMapImage(map, width, getBestScaleForMapImage(map, width), drawScale, scaleLabel);
    }
     ...
     */
    @Override
    public BufferedImage getBoundaryMapImage(String boundaryId, int width, int height,
        ...
        try {
            ...
            return getBoundaryMapImage(map, width, scale, drawScale, scaleLabel);
        } catch (Exception e) {
            LogUtility.log("Failed to generate boundary map image", e);
            throw new SOLAException(ServiceMessage.OT_WS_BOUNDARY_FAILED_TO_GENERATE_MAP);
        }
    }

    private BufferedImage getBoundaryMapImage(MapContent map, int width, double scale,
        ...
        try {
            ...
            return getBoundaryMapImage(map, scale, drawScale, scaleLabel);
        } catch (Exception e) {
            LogUtility.log("Failed to generate boundary map image", e);
            throw new SOLAException(ServiceMessage.OT_WS_BOUNDARY_FAILED_TO_GENERATE_MAP);
        }
    ...
    private BufferedImage getBoundaryMapImage(MapContent map, double scale, boolean drawScale, String scaleLabel)
        ...
        // Draw north arrow
        BufferedImage arrow = ImageIO.read(CommunityBoundaryMapImageEJB.class.getResourceAsStream(resourcesPath + "north_arrow.png"));
